...
contract token {
    uint256 private _certificateCnt = 1;    // 증명서 총 개수
    ...
    function issue(uint256 studentID, string memory issueDate) public {
        ...
        _certificates[studentID].id = _certificateCnt++;
        ...
        hashInput += uint256(keccak256(abi.encode(_certificates[studentID].id)));
        _certificates[studentID].issueHash = keccak256(abi.encode(hashInput));
    }
    function showIssue(uint256 studentID) public view returns (bytes32, uint256, string memory, string memory, string memory, string memory, string memory) {
        require(_isActive[studentID] == 2, "Not Yet Graduate");
        if (_isValid[studentID] == true)
            return (_certificates[studentID].issueHash, studentID, _members[studentID].season, _members[studentID].name, _members[studentID].startDate, _members[studentID].endDate, _certificates[studentID].issueDate);
        else
